...
class ReceptionistController extends Controller {
    ...
    public function dashboard() {
        ...
        $hospital_id = $stmt->fetchColumn() ?: null;

        $data['appointments'] = $appointmentModel->getTodayAppointments(null, $hospital_id);
    ...
    public function mark_paid() {
        ...
        $stmt = $this->db->prepare("UPDATE appointments SET status = 'confirmed' WHERE id = :id");
        $stmt->execute([':id' => $id]);
        
        // Add a payment record (Simulating receiving fee)
        $payStmt = $this->db->prepare("INSERT INTO payments (appointment_id, amount, status, transaction_id) VALUES (:aid, 500, 'paid', :tid)");
        $payStmt->execute([
            ':aid' => $id,
            ':tid' => 'CASH-' . rand(100000, 999999)
        ]);

        echo "<script>alert('Payment received! Status updated to Confirmed.'); window.location='?route=receptionist/dashboard';</script>";
    }
