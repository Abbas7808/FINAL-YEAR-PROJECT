...
class HospitalAdminController extends Controller {
    ...
    public function dashboard() {
        try {
            ...
            // Total Appointments booked for Today (strict hospital scope)
            $stmt = $this->db->prepare("SELECT COUNT(*) FROM appointments WHERE hospital_id = :hid AND DATE(date) = CURDATE()");
            $stmt->execute([':hid' => $this->hospital_id]);
            $data['appointments_today'] = (int)$stmt->fetchColumn();
            
            // Pending Appointments count (strict hospital scope)
            $stmt = $this->db->prepare("SELECT COUNT(*) FROM appointments WHERE hospital_id = :hid AND status = 'pending'");
            $stmt->execute([':hid' => $this->hospital_id]);
     ...
     * Only allows updating appointments that belong to this hospital.
     */
    public function update_appointment() {
        ...
        if ($id && in_array($status, $allowed)) {
            try {
                // Verify the appointment belongs to this hospital before updating
                $checkStmt = $this->db->prepare("SELECT id FROM appointments WHERE id = :id AND hospital_id = :hid");
                $checkStmt->execute([':id' => $id, ':hid' => $this->hospital_id]);
                
                if ($checkStmt->fetch()) {
                    $stmt = $this->db->prepare("UPDATE appointments SET status = :status WHERE id = :id AND hospital_id = :hid");
                    $stmt->execute([
                        ':status' => $status, 
                        ':id'     => $id, 
                    ...
                    $_SESSION['error'] = "Appointment not found or does not belong to your hospital.";
                }
            } catch (Exception $e) {
    ...
    public function appointments() {
        ...
        try {
            // Count total appointments for this hospital only (strict scope)
            $countStmt = $this->db->prepare(
                "SELECT COUNT(*) FROM appointments WHERE hospital_id = :hid"
            );
            $countStmt->execute([':hid' => $this->hospital_id]);
            $total = (int)$countStmt->fetchColumn();

            // Fetch paginated appointments with all needed fields including time_slot
            $stmt = $this->db->prepare("
                SELECT a.id, a.date, a.time_slot, a.status, a.type, a.token_number,
                ...
                ORDER BY a.id DESC, a.date DESC
