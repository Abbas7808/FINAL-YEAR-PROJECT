 ...
 * ==========================================================================
 * This controller manages the entire Med Nova platform from the super admin side.
 * It provides administrative functions including:
 * 1. Global Dashboard Analytics & Stats
 * 2. Doctor Management (Add, Edit, Delete, Affiliations)
 * 3. Hospital & Clinic Management (Subscription Toggle, Creation, Slugs)
 * 4. Regional & User Access Configuration
 * 5. Platform Settings (Data Import/Export, Password Reset, Hard Resets)
 * 
 * Access Control Shield: Enforces role verification on instantiation.
 */
...
class AdminController extends Controller {
     ...
     * Restricts access to authorized Super Admins.
     */
    ...
    /**
     * Action: Renders the Global Dashboard Analytics.
     * Computes totals for hospitals, clinics, subscriptions, and doctors.
     */
    public function dashboard() {
        // 1. Count total registered hospitals
        $stmt = $this->db->query("SELECT COUNT(*) FROM hospitals WHERE type = 'Hospital'");
        $data['total_hospitals'] = $stmt->fetchColumn();

        // 2. Count total registered clinics
        $stmt = $this->db->query("SELECT COUNT(*) FROM hospitals WHERE type = 'Clinic'");
        $data['total_clinics'] = $stmt->fetchColumn();

        // 3. Count active hospital subscriptions
        ...
        // 6. Query the 5 most recently registered doctors
        ...
        $this->view('admin/dashboard', $data); // Load dashboard view
    }

    /**
     * Action: Lists doctors on the platform.
     * Supports filtering by medical facility type or specific hospital.
     */
    public function doctors() {
        $filter_type = $_GET['type'] ?? ''; // Filter: 'Hospital' or 'Clinic'
        ...
        // Fetch full hospital dropdown checklist for admin forms
    ...
    /**
     * Action: Renders detailed profile of a selected doctor.
     */
    public function doctor_details() {
        ...
        $data['reviews'] = []; // Future review table integration stub
        $data['page_title'] = "Doctor Details";
        $this->view('admin/doctor_details', $data);
    }

    /**
     * Action: Renders doctor addition view.
     */
    ...
    /**
     * Action: Saves a new doctor profile and creates their system credentials.
     */
    public function store_doctor() {
         if ($_SERVER['REQUEST_METHOD'] == 'POST') {
             ...
             // 1. Create entry in users auth table
             ...
             // 2. Create entry in doctors details metadata table
             ...
             // 3. Create entry in hospital affiliations table
             if ($hospital_id) {
    ...
    /**
     * Action: Renders edit form for a doctor's profile.
     */
    ...
    /**
     * Action: Updates doctor details inside a secure database transaction.
     */
    public function update_doctor() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                $this->db->beginTransaction(); // Transaction protects database integrity
                
                // 1. Update Core user details (and hash password if modified)
                ...
                // 2. Update metadata profile details
                ...
                // 3. Clear existing affiliations and create new connection
                ...
                $this->db->rollBack(); // Revert modifications on error
    ...
    /**
     * Action: Deletes a doctor user and related records from affiliations/details.
     */
    public function delete_doctor() {
        if (isset($_GET['id'])) {
            ...
            try {
                ...
                if ($user_id) {
                    // Delete hospital affiliation linking records
    ...
    /**
     * Action: Saves a new receptionist user credentials.
     */
    ...
    /**
     * Action: Lists all receptionist users.
     */
    ...
    /**
     * Action: Deletes receptionist user.
     */
    ...
    /**
     * Action: Lists all geographical regions.
     */
    ...
    /**
     * Action: Lists all registered hospitals.
     */
    ...
    /**
     * Action: Lists all registered clinics.
     */
    ...
    /**
     * Action: Creates a new hospital/clinic network record and user admin login.
     * Generates a URL slug and handles file uploads for profiles.
     */
    public function store_hospital() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Generate clean URL slug from the hospital name
            ...
            // Defaults for new subscription plans
            $plan_type = 'monthly';
            $status = 'active';
            ...
            if(!empty($name) && !empty($phone) && !empty($password)){
                try {
                    ...
                    // 1. Create administrator credentials account in users table
                    $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                    ...
                    // 2. Create the hospital details linking user_id
    ...
    /**
     * Action: Deletes hospital listing.
     */
    ...
    /**
     * Action: Updates hospital profile data and processes profile logo replacements.
     */
    public function update_hospital() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Handle image uploads if a new file is attached
            if(isset($_FILES['image']) && $_FILES['image']['error'] == 0){
                $target_dir = "assets/uploads/hospitals/";
                if (!file_exists($target_dir)) {
                    mkdir($target_dir, 0777, true);
    ...
    /**
     * Action: Toggles subscription status (Active / Inactive) of facilities.
     * Inactive networks cannot be selected by patients for appointments.
     */
    ...
    /**
     * Action: Fetches statistics for a specific hospital (doctors counts, appointment volumes).
     */
    ...
    /**
     * Action: Performs a universal search across doctors and hospitals.
     */
    public function search() {
        ...
        if (empty(trim($q))) {
            ...
            // Search hospitals matching query
    ...
    /**
     * Action: Lists users and manages facility link assignments.
     */
    ...
    /**
     * Action: Adds a user account and configures dynamic facility linking for hospital admins.
     */
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($check->fetch()) {
                ...
                try {
                    ...
                    // If role is hospital admin, link to the designated hospital
    ...
    /**
     * Action: Modifies user role permissions and updates hospital links.
     */
    ...
    /**
     * Action: Renders settings, loading row metrics for database tables.
     */
    public function settings() {
        $stmt = $this->db->query("SELECT id, name, phone, role, created_at FROM users ORDER BY role ASC, name ASC");
        $data['users'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Fetch Row Counts to display dashboard storage metrics
        $data['stats'] = [];
        $tables = [
            'users' => 'users',
            'hospitals' => 'hospitals',
    ...
    /**
     * Action: Exports selected database tables as a JSON backup payload download.
     */
    ...
    /**
     * Action: Restores database tables from a uploaded JSON backup.
     */
    ...
    /**
     * Action: Resets a selected user's password to a custom string.
     */
    ...
    /**
     * Action: Resets all transaction logs and listings.
     * Preserves only Administrator credentials. Dangerous zone action.
     */
